...
import { ContentTypeEnum, formatRequestDate, joinTimestamp, setObjToUrlParams } from './utils';
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: async (res, options) => {
    ...
    if (res.data.code === 401) {
      ...
      await MessagePlugin.warning(t('components.invalid_token'));
      return res;
    }
    ...
    const { data } = res;
    if (!data) {
      throw new Error(t('components.api_error'));
    }
    ...
    throw new Error(t('components.api_error_code') + code);
  },
